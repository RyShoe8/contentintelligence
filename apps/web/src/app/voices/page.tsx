...
function personaStatusLabel(status: string): string {
...
export default async function VoicesPage({
  ...
  return (
    <div className="space-y-6">
      ...
      {sp.generating === "1" ? (
        <p className="rounded border border-primary/30 bg-surface px-3 py-2 text-sm text-[var(--fg)]">
          Persona generation started. Refresh this page in a minute to see results.
        </p>
      ) : null}
      {errorMsg ? (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-[var(--muted)]">
                    {personaStatusLabel(v.persona_status)}
      ...
      <section className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
        ...
        <form action={saveVoiceAction} className="space-y-4">
          {activeVoice ? (
          ...
          <label className="flex flex-col gap-1 text-sm">
            Persona template
            <span className="text-xs text-[var(--muted)]">
              Generated by AI from your sources, then editable. Used when building posts.
            </span>
            <textarea
              name="persona"
              rows={12}
