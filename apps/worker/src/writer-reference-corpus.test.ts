...
describe("formatReferenceCorpusForPrompt", () => {
  it("formats sections with url headers", () => {
    const text = formatReferenceCorpusForPrompt([
      { url: "https://a.example", text: "Alpha facts." },
      { url: "https://b.example", text: "Beta facts." },
    ]);
    assert.match(text, /### Reference 1: https:\/\/a\.example/);
    assert.match(text, /Alpha facts\./);
    assert.match(text, /### Reference 2: https:\/\/b\.example/);
  });
